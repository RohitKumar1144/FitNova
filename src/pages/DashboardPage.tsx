...
export default function DashboardPage() {
  ...
  const [activityScore, setActivityScore] = useState<ActivityScoreData>({
    hasHistory: false,
    totalSessions: 0,
    workoutsLast7Days: 0,
    consistency: 0,
  ...
  // Load profile, stats, latest plan, and activity score
  useEffect(() => {
    async function loadDashboardData() {
      ...
      try {
        const [profileRes, statsRes, planRes, scoreRes] = await Promise.all([
          getProfile(user.id),
          getDashboardStats(user.id),
          getLatestWorkoutPlan(user.id),
          getActivityScore(user.id),
        ])
        ...
        if (scoreRes.data) {
          setActivityScore(scoreRes.data)
        }
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 selection:bg-emerald-500 selection:text-slate-950 flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {/* 3.5. FITNOVA ACTIVITY SCORE CARD */}
        <div className="rounded-3xl bg-gradient-to-br from-slate-900 via-slate-900/90 to-slate-950 border border-emerald-500/20 p-6 sm:p-8 shadow-2xl relative overflow-hidden">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            
            {/* Left: Score Badge & Snapshot */}
            <div className="flex items-start sm:items-center gap-5">
              <div className="relative shrink-0">
                <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl bg-gradient-to-br from-emerald-500/10 via-teal-500/10 to-slate-900 border-2 border-emerald-500/30 flex flex-col items-center justify-center shadow-lg shadow-emerald-500/10">
                  {activityScore.hasHistory ? (
                    <>
              ...
              <div className="space-y-1.5">
                <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[11px] font-bold uppercase tracking-wider">
                  <Sparkles className="w-3 h-3" />
                  <span>FitNova Activity Score</span>
                </div>
                <h3 className="text-xl sm:text-2xl font-black text-white tracking-tight">
                  {activityScore.hasHistory
                    ? 'Your activity snapshot'
                    : 'Start your activity score'}
                </h3>
                <p className="text-xs sm:text-sm text-slate-400 max-w-xl leading-relaxed">
                  {activityScore.hasHistory
                    ? 'Your Activity Score combines consistency, exercise form, workout completion, and progression.'
                    : 'Complete your first workout to start your Activity Score.'}
                </p>
              </div>
            </div>
            ...
            {activityScore.hasHistory ? (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full lg:w-auto">
                {/* Component 1: Consistency (30%) */}
                <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800 text-center min-w-[95px]">
                  <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider block mb-1">
                    Consistency
                  </span>
                  <span className="text-lg sm:text-xl font-black text-white block">
                    {activityScore.consistency}
                  </span>
                ...
                {/* Component 3: Completion (20%) */}
                <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800 text-center min-w-[95px]">
                  <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider block mb-1">
                    Completion
                  </span>
                  <span className="text-lg sm:text-xl font-black text-white block">
                    {activityScore.completion}
                  </span>
                ...
                {/* Component 4: Progression (20%) */}
                <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800 text-center min-w-[95px]">
                  <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider block mb-1">
                    Progression
                  </span>
                  <span className="text-lg sm:text-xl font-black text-white block">
                    {activityScore.progression}
                  </span>
                  <span className="text-[9px] text-teal-400/90 font-medium block">
                    20% Weight
                  </span>
              ...
              <div className="flex items-center">
                <Link
                  to="/workout-session"
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 text-xs sm:text-sm font-bold shadow-lg shadow-emerald-500/20 transition duration-200"
                >
