...
export default function WorkoutSessionPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col">
      ...
      <nav className="border-b border-slate-800/80 bg-slate-950/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-14 sm:h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5 sm:gap-3">
            <Link
              to="/dashboard"
              className="p-1.5 sm:p-2 rounded-xl bg-slate-900 border border-slate-800 text-slate-400 hover:text-white transition cursor-pointer"
              title="Back to Dashboard"
            >
              <ArrowLeft className="w-4 h-4" />
            </Link>

            <Link to="/dashboard" className="flex items-center gap-2 sm:gap-2.5">
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-gradient-to-br from-emerald-400 to-teal-600 flex items-center justify-center shadow-md shadow-emerald-500/20">
                <Activity className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-950 stroke-[2.5]" />
              </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {/* Timer Badge */}
            <div className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-xs font-mono font-bold text-slate-200 shadow-inner">
            ...
            {sessionStatus !== 'ended' && (
              <button
                type="button"
                onClick={handleReset}
                className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-400 hover:text-white text-xs font-semibold transition cursor-pointer"
      ...
      {/* Main Container */}
      <main className="flex-1 max-w-7xl w-full mx-auto px-2.5 sm:px-6 lg:px-8 py-2 sm:py-6 flex flex-col justify-center">
        ...
        {sessionStatus === 'ended' ? (
          ...
          <div className="space-y-2.5 sm:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4 pb-1 sm:pb-2">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 sm:px-2.5 py-0.5 rounded-full">
                    AI Form Coach
                  </span>
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-1 sm:mt-1.5">
                  ...
                  {sessionStatus === 'idle' && (
                    <div
                      ...
                      className="flex items-center gap-1 bg-slate-900/90 border border-slate-800 p-0.5 sm:p-1 rounded-xl"
                    >
                      <button
                        type="button"
                        role="radio"
                        ...
                        className={`px-2.5 sm:px-3 py-1 rounded-lg text-[11px] sm:text-xs font-bold transition cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-400 ${
                          exerciseType === 'squat'
                ...
                <p className={`text-xs sm:text-sm text-slate-400 mt-0.5 sm:mt-1 ${sessionStatus !== 'idle' ? 'hidden sm:block' : ''}`}>
                  {exerciseType === 'bicep_curl'
              ...
              <div className="flex items-center gap-2 pt-1 sm:pt-0">
                {sessionStatus === 'idle' && (
                  <button
                    ...
                    className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 sm:px-6 py-2 sm:py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs sm:text-sm shadow-lg shadow-emerald-500/20 transition cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-400"
                  >
                ...
                {sessionStatus === 'active' && (
                  <>
                    <button
                      type="button"
                      onClick={handlePause}
                      className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-400 font-bold text-xs sm:text-sm transition cursor-pointer focus:outline-none focus:ring-2 focus:ring-amber-400"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2.5 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 text-rose-400 font-bold text-xs sm:text-sm transition cursor-pointer focus:outline-none focus:ring-2 focus:ring-rose-400"
                    >
                      <Square className="w-3 h-3 sm:w-3.5 sm:h-3.5 fill-rose-400" />
                ...
                {sessionStatus === 'paused' && (
                  <>
                    <button
                      ...
                      className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs sm:text-sm shadow-lg shadow-emerald-500/20 transition cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-400"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] xl:grid-cols-[1fr_300px] gap-2.5 sm:gap-6 items-start">
              {/* Camera / Setup Viewport */}
              <div className="space-y-2 sm:space-y-3">
                {sessionStatus === 'idle' ? (
                  <div className="w-full aspect-[4/3] sm:aspect-[16/10] max-h-[560px] rounded-2xl sm:rounded-3xl bg-slate-950 border border-slate-800 flex flex-col items-center justify-center p-4 sm:p-10 text-center shadow-2xl relative overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-b from-emerald-500/5 to-transparent pointer-events-none" />

                    <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 mb-2 sm:mb-4 shadow-lg shadow-emerald-500/10">
                      <Dumbbell className="w-6 h-6 sm:w-8 sm:h-8" />
                    </div>

                    <h3 className="text-lg sm:text-2xl font-black text-white mb-1.5 sm:mb-2">
                      {exerciseType === 'bicep_curl'
                        ? 'Ready to Curl?'
                        : exerciseType === 'pushup'
                        ? 'Ready for Push-ups?'
                    ...
                    <p className="text-xs sm:text-sm text-slate-400 max-w-md mb-3 sm:mb-6 leading-relaxed">
                      {exerciseType === 'bicep_curl'
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 text-[11px] sm:text-xs text-slate-400 mb-4 sm:mb-6">
                    ...
                    <button
                      type="button"
                      onClick={handleStartWorkout}
                      className="inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-2.5 sm:py-3.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs sm:text-base shadow-xl shadow-emerald-500/25 transition duration-200 hover:scale-[1.02] active:scale-[0.98] cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-400"
                    >
                  ...
                  <div className="relative">
                    <CameraFeed
                      ...
                      className="w-full shadow-2xl shadow-emerald-950/20 aspect-[4/3] sm:aspect-[16/10] max-h-[40vh] sm:max-h-[560px] rounded-2xl sm:rounded-3xl"
                    />

                    {/* Quick In-Video Control Bar for Convenient Desktop Access */}
                    <div className="absolute bottom-2 sm:bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-2xl bg-slate-950/85 backdrop-blur-md border border-slate-800 z-10 shadow-lg">
                      {sessionStatus === 'active' ? (
                        <button
                          type="button"
                          onClick={handlePause}
                          className="inline-flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 py-1 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 text-amber-400 font-bold text-xs transition cursor-pointer"
                        >
                        ...
                        <button
                          type="button"
                          onClick={handleResume}
                          className="inline-flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 py-1 rounded-xl bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 font-bold text-xs transition cursor-pointer"
                        >
                      ...
                      <button
                        type="button"
                        onClick={handleEndWorkout}
                        className="inline-flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 py-1 rounded-xl bg-rose-500/20 hover:bg-rose-500/30 text-rose-400 font-bold text-xs transition cursor-pointer"
                      >
              ...
              {/* Side Panel / Mobile Bottom Grid — Rep Counter & Form Feedback */}
              <div className="grid grid-cols-2 lg:flex lg:flex-col gap-2.5 sm:gap-4 lg:gap-5">
                <RepCounter
